...
local function build_layout_tree(data, stk)
    for _,win in ipairs(data.wins) do
...
local function set_each_win(data)
    local final_win = nil
    for i = 1, #data.wins do
        local winnr = i
        local winId = winnr_to_winid(winnr)
        assert(winId ~= nil)
        assert (data.wins[i].winnr == winnr, f("%s vs %s", data.wins[i].winnr, winnr))
        local file = data.wins[i].file
        ...
        pcall(function() vim.api.nvim_win_set_cursor(winId, data.wins[i].cursor) end)
...
local restore_layout = function(data)
    ...
    data.wins = vim.tbl_filter(filter_fn, data.wins)
    ...
    local root = stk:top()
    stk:pop()
    -- log(f("root: %s, data: %s", vim.inspect(root), vim.inspect(data.wins)))
