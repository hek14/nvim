...
M.print = function (...)
  ...
  for _, input in ipairs(arg) do
    if not M.cnl_valid() or not M.buf_valid() then M.validate()
...
M.exists = function ()
  return M.win_valid() and M.buf_valid() and M.cnl_valid()
end
...
M.create_term = function ()
  M.buf = not M.buf_valid() and a.nvim_create_buf(false, true) or M.buf
  M.chan = a.nvim_open_term(M.buf, {})
  return M.buf
end
